...
public class ThreatService {
    ...
    public DeviceStatsResponse getDeviceStats() {
        ...
        List<ThreatReport> recentThreats = threatReportRepository.findAll()
            .stream()
            .filter(t -> t.getReportedAt().isAfter(last24Hours))
            .toList();
    ...
    public List<AttackSurfaceDataPoint> getAttackSurfaceData() {
        ...
        for (int i = 11; i >= 0; i--) {
            ...
            long threatCount = threatReportRepository.findAll().stream()
                .filter(t -> t.getReportedAt().isAfter(timeRangeStart) && 
                            t.getReportedAt().isBefore(timePoint))
                .count();
